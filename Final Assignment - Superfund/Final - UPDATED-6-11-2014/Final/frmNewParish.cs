...
namespace Final
{
    public partial class frmNewParish : Form
    {
        ...
        {
            ParishLB.DataSource = db.Parishes.Select(p => p.ParishID).Distinct().ToList();
        ...
        private void SaveSubjbtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MessageBox.Show("Contact Successfully Added to Parishes!");

                ParishTB.Focus();
            }
            catch (Exception)
            {
                MessageBox.Show("Parish entered - Cannot Be Saved");
            }
        }
